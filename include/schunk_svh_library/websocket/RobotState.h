...
/*!
 * \brief The RobotState class is a helper class to hold the relevant information about the current
 * robot state A robo has many axes, a movement state, fault states for every joint. In Short this
 * class was meant to be used for the SCHUNK LWA4-P Together with canopen. However it might also be
 * used with other arms
 */
...
public:
   ...
   * \param ticks_per_second Number of simulation stepds per second if the simulation should be
   * used. Default = 100. Not needed if you use real data
   */
  RobotState();
  ...
  //! Shuffles the Error values to contain random values from the pool of possible errors (used for
  //! simulation)
  ...
  /*!
   * \brief setJointPositions Sets the current joint positionts. Only num_axes elements will be
   * used. Unset element will retain their previous value \param joint_positions new joint positions
   * in your format, this will be multiplied by the toRadMultiplier
   */
   ...
   * \param joint_position Current Position in your format, this will be multiplied by the
   * toRadMultiplier \param joint index of the joint (starts with zero, obviously)
   */
  ...
  /*!
   * \brief setJointVelocties Sets the current joint velocities. Only num_axes elements will be
   * used. Unset element will retain their previous value \param joint_velocities new joint
   * velocities in your format, this will be multiplied by the toRadMultiplier
   */
   ...
   * \param joint_velocit< new joint velocities inyour format, this will be multiplied by the
   * toRadMultiplier
   */
  void setJointVelocity(const double& joint_velocity, const size_t& joint);

  /*!
   * \brief setJointErrors Sets the current joint error state. Only num_axes elements will be used.
   * Unset element will retain their previous value \param joint_errors new joint error state
   */
  ...
  /*!
   * \brief setJointErrors Sets the current joint errors in a simplified way (only fault or no
   * fault) if no further information is available \param faults true if joint had a fault, false
   * otherwise
   */
  void setJointErrors(const std::vector<bool>& faults);

  /*!
   * \brief setJointError Simplified version of error setting for a particular joint if only fault
   * or no fault is supported \param fault true if joint had a fault, false otherwise \param joint
   * the joint to set the error for
   */
  ...
  // virtual void setState(const std::vector<double> &joint_positions,const std::vector<double>
  // &joint_velocities,const std::vector<bool> &faults,const MovementState &movement_state);


  void setTps(unsigned int tps) { m_tps = tps; }

  size_t getNumAxes() const;

  double getInputToRadFactor() const { return m_input_to_rad_factor; }
...
protected:
  ...
  //! Hints are identifiers for a diagnostics gui indicating what is wrong with a certain setup,
  //! these hints need to be represented in the config of the delivered diagnostics page
  std::set<int> m_hints;
};
